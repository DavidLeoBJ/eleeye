...
static void PopPvLine(int nDepth = 0, int vl = 0) {
  ...
  fflush(stdout);
  if (nDepth == 0) {
    //fprintf(stderr, "nDepth is 0, returning early!");
...
void SearchMain(int nDepth) {  
  fprintf(stderr, "SearchMain entered, nDepth=%d", nDepth);
  ...
  if (g_useBook) {
    ...
    if (nBookMoves > 0) {
      ...
      if (Search.pos.RepStatus(3) == 0) {
        ...
        // Search.pos.UndoMakeMove();
        // if (nBookMoves > 0) {
        //     Search.mvPonder = bks[0].wmv;   // ★ 开局库给的对方最佳应着
        // } else {
        //     Search.mvPonder = 0;
        // }
...
// #endif
  ...
  for (i = 1; i <= nDepth; i ++) {
    ...
    // 8. 搜索根结点
    vl = SearchRoot(i);
    vlLast = vl;   // ← 移到这里！每次迭代都记录最新分数。目的是：解决CCHESS_A3800模式没有评分输出的问题。作者原来认为这个模式下不需要评分输出。
    if (Search2.bStop) {
      break;
    }
  ...
  if (Search2.wmvPvLine[0] != 0) {
    int g_pvDebug[10] = {0};
    // ===== 调试：看 PopPvLine 之前 wmvPvLine 里有什么 =====
    printf("BEFORE PopPvLine: [%d, %d, %d, %d, %d]\n",
        Search2.wmvPvLine[0], Search2.wmvPvLine[1], Search2.wmvPvLine[2],
        Search2.wmvPvLine[3], Search2.wmvPvLine[4]);
    PopPvLine();
    // ===== 调试：看 PopPvLine 之后 wmvPvLine 里有什么 =====
    printf("AFTER PopPvLine: [%d, %d, %d, %d, %d]\n",
        Search2.wmvPvLine[0], Search2.wmvPvLine[1], Search2.wmvPvLine[2],
        Search2.wmvPvLine[3], Search2.wmvPvLine[4]);

    dwMoveStr = MOVE_COORD(Search2.wmvPvLine[0]);
    printf("bestmove %.4s", (const char *) &dwMoveStr);
    Search.mvResult = Search2.wmvPvLine[0];       // ★ 新增
    if (Search2.wmvPvLine[1] != 0) {   // ← ponder 分支（里面不用动）
        dwMoveStr = MOVE_COORD(Search2.wmvPvLine[1]);
        printf(" ponder %.4s", (const char *) &dwMoveStr);
        Search.mvPonder = Search2.wmvPvLine[1];
    }
