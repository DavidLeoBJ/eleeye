...
void EnsurePreGenInited() {
  std::call_once(g_pregen_flag, []() { PreGenInit(); });
}
...
bool isEndgameFen(const std::string &rawFen) {
  ...
  return redPieceCount <= 5 || strstr(rawFen.c_str(), "P3/") ||
         strstr(rawFen.c_str(), "P4/") || strstr(rawFen.c_str(), "P5/");
}
  ...
  while (*p && *p != ' ') {
    if (*p == '/') {
      ...
      if (*p >= 'A' && *p <= 'Z') {
        ...
        switch (*p) {
        case 'K':
          pos.ucpcSquares[sq] = 1;
          break; // 红帅
        case 'R':
          pos.ucpcSquares[sq] = 2;
          break; // 红车
        case 'C':
          pos.ucpcSquares[sq] = 3;
          break; // 红炮
        case 'N':
          pos.ucpcSquares[sq] = 4;
          break; // 红马
        case 'B':
          pos.ucpcSquares[sq] = 5;
          break; // 红相
        case 'A':
          pos.ucpcSquares[sq] = 6;
          break; // 红仕
        case 'P':
          pos.ucpcSquares[sq] = 7;
          break; // 红兵
        }
      } else {
        // 黑方子力编码（和象眼原生一致）
        switch (*p) {
        case 'k':
          pos.ucpcSquares[sq] = 17;
          break; // 黑将
        case 'r':
          pos.ucpcSquares[sq] = 18;
          break; // 黑车
        case 'c':
          pos.ucpcSquares[sq] = 19;
          break; // 黑炮
        case 'n':
          pos.ucpcSquares[sq] = 20;
          break; // 黑马
        case 'b':
          pos.ucpcSquares[sq] = 21;
          break; // 黑象
        case 'a':
          pos.ucpcSquares[sq] = 22;
          break; // 黑士
        case 'p':
          pos.ucpcSquares[sq] = 23;
          break; // 黑卒
        }
      }
      x++; // 处理完一个子力，x向右移动一格
