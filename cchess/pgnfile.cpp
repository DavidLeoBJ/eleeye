...
// 废除镜像坐标系（'9' + RANK_TOP - c→ c - '0'），兼容两种 ICCS 格式（H2-E2/ H2E2），不破坏中文记谱路径
// 不能使用inline:inline+ 多 cpp + 静态链接 = ODR 违规，链接器会偷偷用旧版本
int ICCS_MOVE(const char *szIccsStr)
{
    // #error ">>> ICCS_MOVE COMPILED <<<"
    int sqSrc, sqDst;
    int fileSrc = szIccsStr[0] - 'A';
    int rankSrc = szIccsStr[1] - '0'; // ✅ 关键修复：去掉镜像
    int fileDst, rankDst;

    // 支持 H2-E2（带连字符）和 H2E2（无连字符）
    if (szIccsStr[2] == '-')
    {
        fileDst = szIccsStr[3] - 'A';
        rankDst = szIccsStr[4] - '0'; // ✅ 关键修复：去掉镜像
    }
    else
    {
        fileDst = szIccsStr[2] - 'A';
        rankDst = szIccsStr[3] - '0'; // ✅ 关键修复：去掉镜像
    }

    sqSrc = COORD_XY(fileSrc, rankSrc);
    sqDst = COORD_XY(fileDst, rankDst);
    return MOVE(sqSrc, sqDst);
}
    ...
    while (!(feof(fp) && bReturned))
    {
        ...
        if (bDetail)
        {
            if (nRemLevel > 0)
            {
                ...
                {
                    nRemLevel += (*lpLineChar == '(' || *lpLineChar == '{' ? 1 : *lpLineChar == ')' || *lpLineChar == '}' ? -1
                                                                                                                          : 0);
                    if (nRemLevel == 0)
                    {
                        szRem[nRemLen] = '\0';
                            ...
                            if ((*lpLineChar >= 'A' && *lpLineChar <= 'Z') || (*lpLineChar >= 'a' && *lpLineChar <= 'z') || *lpLineChar == '+' || *lpLineChar == '-' || *lpLineChar == '=')
                            {
                                ...
                                if (TryMove(pos, nStatus, mv))
                                {
                                    ...
                                    if (nCounter < MAX_COUNTER)
                                    {
                                        ...
                                        {
                                            // ICCS: H2-E2 (5 chars) or H2E2 (4 chars)
                                            if (lpLineChar[2] == '-')
                                            {
                                                lpLineChar += 5; // H2-E2
                                            }
        ...
        else
        {
            ...
            else if (szLineStr[0] == '[')
            {
                ...
                {
                    fprintf(stderr, "[GETLABEL] szRem=[%s] line=[%s]\n", szRem, szLineStr);
                    if (StrEqv(szRem, "WXF"))
                    {
                        nNotation = 1;
                    }
                    ...
                    {
                        nNotation = 2;
                        fprintf(stderr, "[FORMAT] ICCS detected, nNotation=2\n");
                    }
                    else
                    {
                        nNotation = 0;
